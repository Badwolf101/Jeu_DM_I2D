...
backpack = []
sizebackpack = 2
moral = 100 #TODO <-------------------- super idée, a implementer
...
def input_check(message, data_type, valid_input):
  """
    Demande à l'utilisateur d'entrer une réponse qui correspond aux critères spécifiés.
    
    Parameters:
    message (str): Le message personnalisé qui sera envoyé à l'utilisateur.
    data_type (type): Le type de données attendu pour la réponse.
    valid_input (tuple): Une valeur ou un intervalle de valeurs acceptables pour la réponse.
    
    Returns:
    La réponse de l'utilisateur qui correspond aux critères spécifiés.
    """
...
#TODO séquence grotte


#fin du jeu :

#TODO faut utiliser la lampe et créer 2 fin
#* choisir un nb, ce nb deveint le nb à réaliser avec des dé pour réussir à passer l'épreuve (fonction renvoie juste pass ou non). A la limite un 2eme nb pour le nb d'éssai
#* NOTE : peut etre intéressant de fuir le lion pour trouver la grotte et la personne disparue, la non-utilisation de la lampe peut etre un indice d'un sénario non découvert
#? implimenter le moral
#? implimenter l'écriture carcactère par caractère